...
- Saves processed h5ad files with CN annotations for each tile
- Saves composition CSV for later visualization

Note: Visualizations are generated by cn_unified_kmeans_groups.py
"""
...
class UnifiedCellularNeighborhoodDetector:
    ...
    def __init__(self, tiles_directory: str, output_dir: str = 'cn_unified_results'):
        ...
        # Create subdirectories (only processed_h5ad and unified_analysis for composition CSV)
        for subdir in ['processed_h5ad', 'unified_analysis']:
    ...
    def run_full_pipeline(
        ...
        # Step 5: Compute composition (save for later visualization)
        composition, composition_zscore = self.compute_unified_cn_composition(celltype_key=celltype_key)
        
        # Save composition CSV for later use
        comp_path = self.output_dir / 'unified_analysis' / 'unified_cn_composition.csv'
        comp_path.parent.mkdir(exist_ok=True)
        composition.to_csv(comp_path)
        print(f"  ✓ Saved composition to: {comp_path}")

        # Step 6: Save processed data (h5ad files only)
        ...
        print("\nProcessed h5ad files are ready for visualization!")
        print("Run cn_unified_kmeans_groups.py to generate visualizations.")
...
def main():
    ...
    parser.add_argument(
        '--n_clusters', '-n', type=int, default=13,  # default=7
